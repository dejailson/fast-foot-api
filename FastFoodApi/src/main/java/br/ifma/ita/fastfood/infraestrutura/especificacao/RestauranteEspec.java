...
import br.ifma.ita.fastfood.dominio.db.modelo.Endereco_;
import br.ifma.ita.fastfood.dominio.db.modelo.Restaurante;
import br.ifma.ita.fastfood.dominio.db.modelo.Restaurante_;
...
public class RestauranteEspec {
	...
	public static class Construtor extends OperadorLogico<Restaurante>{
		List<Specification<Restaurante>> predicados= new ArrayList<>();
		...
		public Construtor cidade() {
			if (filtro.getCodigoCidade() !=null) {
				predicados.add(temCidadeIdIgual(filtro.getCodigoCidade()));
			}
			return this;
		}
		...
		public Construtor servicoEntrega() {
			if (filtro.getCodigoServicoEntrega() !=null) {
				predicados.add(temServicoEntregaIdIgual(filtro.getCodigoServicoEntrega()));
			}
			return this;
		}
		...
		public Construtor estaAberto() {
			...
			if (filtro.getSituacao() !=null) {
				if (filtro.getSituacao().estaAberto()) {
					predicados.add(aberto());
				}else {
					predicados.add(fechado());
				}
			}
